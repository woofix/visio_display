...
- The app container must not mount `/var/run/docker.sock`. Docker access is isolated in the internal `updater` service, authenticated with `UPDATER_API_TOKEN`, not publicly exposed, and limited to fixed allowlisted operations. Treat super-admin access and updater-level Docker access as sensitive operational privileges.
